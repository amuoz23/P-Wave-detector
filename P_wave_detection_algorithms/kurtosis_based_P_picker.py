...
def plot_trigger(trace, cft, prev_cft, thr_on, thr_off, batch, stn, show=True): #function to plot the trigger on and off values
    ...
    if show:
        ...
        path_temp_2 = "Results/standard_STA_LTA/" + stn + "/" + "True" + "/"
    ...
    if (s[0] == "ENN'"):
        ...
        if (s[1] == time_rec):
            ...
            if (len(st_ENZ) >= 1100 and len(st_ENZ) % 100 == 0):
                ...
                scnl, picks, polarity, snr, uncert = chenPicker.picks(st[0])
                print(('scnl:', scnl))
                print(('picks:', picks))
                ...
                # plot
                summary = ktpicker.KTSummary(chenPicker, st[0])
                # set the plot title
                summary.plot_summary(str(batch), station_name)
                summary.plot_picks(str(batch), station_name)
                batch = batch + 1