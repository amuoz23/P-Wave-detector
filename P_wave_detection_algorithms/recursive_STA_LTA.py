...
global flag_sta_assign, flag_plot_assign #global variables to assign the initial values of sta and lta and to plot the first figure
flag_sta_assign = True
flag_plot_assign = True


def recursive_sta_lta_py(a, nsta, nlta, prev_sta, prev_lta): #function to calculate the sta, lta and charfct values
    global flag_sta_assign
...
while 1:  # loop forever
    ...
    if (s[0] == "ENN'"):
        ...
        if (s[1] == time_rec):
            ...
            if (len(st_ENZ) >= 1100 and len(st_ENZ) % 100 == 0):
                st_main.filter("bandpass", freqmin=0.1, freqmax=20)
                data_pckt = st_main[0].data[len(st_main[0].data) - 1100:]
                ...
                prev_sta, prev_lta, cft = recursive_sta_lta_py(st[0].data, int(sta * df), int(lta * df), prev_sta, prev_lta)
